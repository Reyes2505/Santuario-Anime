...
export default function AnimeDetailPage() {
  ...
  const handleMarcar = async (estado: 'viendo' | 'visto' | 'por_ver') => {
    ...
    // ✅ CON AWAIT
    if (estado === 'viendo') await marcarComoViendo(id as string, 1);
    if (estado === 'visto') await marcarComoVisto(id as string);
    if (estado === 'por_ver') await marcarPorVer(id as string);
  ...
  return (
    <main className="min-h-screen bg-zinc-950 pb-16">
      ...
      <div className="relative h-64 md:h-80 overflow-hidden">
        ...
        <Link href="/" className="absolute top-4 left-4 text-sm text-zinc-400 hover:text-white">
          ← Volver
        </Link>
      </div>
            ...
            <div className="flex gap-2 mt-4">
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-bold ${
                  estadoTracking === 'viendo' ? 'bg-blue-600 text-white' : 'bg-zinc-800 text-zinc-300'
                }`}
              >
                👁️ Viendo
              </button>
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-bold ${
                  estadoTracking === 'visto' ? 'bg-green-600 text-white' : 'bg-zinc-800 text-zinc-300'
                }`}
              >
                ✅ Visto
              </button>
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-bold ${
                  estadoTracking === 'por_ver' ? 'bg-yellow-600 text-white' : 'bg-zinc-800 text-zinc-300'
                }`}
              >
                📌 Por ver
              </button>
